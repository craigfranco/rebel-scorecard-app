        ...
        properties: {
          rows: {
            ...
            items: {
              type: 'object',
              properties: {
                hotel_name: { type: 'string', description: 'Hotel/property name from the first column (usually labeled "Property", "Hotel", or "Name").' },
                month: { type: 'string', description: 'Month name or number if visible in file. Leave blank if not found.' },
                year: { type: 'string', description: 'Year from file (e.g., 2026). Leave blank if not found.' },
                revpar_index_change: { type: 'string', description: 'RevPAR Index % Chg value. Numeric only, no % symbol.' },
                budgeted_gop_actual: { type: 'string', description: 'GOP actual dollars. Leave blank if not in file.' },
                budgeted_gop_target: { type: 'string', description: 'GOP target/budget. Leave blank if not in file.' },
                gop_margin_actual: { type: 'string', description: 'GOP margin %. Leave blank if not in file.' },
                gop_margin_prior: { type: 'string', description: 'GOP margin % prior year. Leave blank if not in file.' },
                gss_actual: { type: 'string', description: 'Guest satisfaction score. Leave blank if not in file.' },
                gss_prior: { type: 'string', description: 'Guest satisfaction score prior year. Leave blank if not in file.' },
              },
            },
          },
        },

...
function PropertyIssueRow({ prop, issues }) {
  const brandColor = getBrandColor(prop.parent_brand);
  return (
    <div className="px-6 py-3.5 flex flex-col sm:flex-row sm:items-center gap-2">
...
function getPropertyIssues(propEntries, prop) {
...
export default function AttentionNeeded({ properties, allEntries, getPeriodMonths, selectedYear }) {
  ...
  const flagged = properties
    .map(prop => {
      const propEntries = allEntries.filter(e => 
        e.property_id === prop.id && periodMonths.includes(e.month) && e.year === selectedYear
      );
      const issues = getPropertyIssues(propEntries, prop);
      return { prop, issues };
    })
    ...
    <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
      ...
      {flagged.length === 0 ? (
        <div className="px-6 py-8 flex items-center gap-3">
          <CheckCircle className="w-5 h-5 text-pass shrink-0" />
          <span className="font-semibold text-sm text-foreground">All properties on track for the selected period.</span>
        </div>
      ) : (
        <div className="divide-y divide-border">
          {flagged.map(({ prop, issues }) => (
            <PropertyIssueRow key={prop.id} prop={prop} issues={issues} />
          ))}
        </div>
      )}
    </div>
